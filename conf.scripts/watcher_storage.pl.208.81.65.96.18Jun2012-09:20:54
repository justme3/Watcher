...
#
	...
	if ( scalar(@arrayKeys) <= 0 )
	{
		print "\n\nERROR: No arrays to process. Exiting.\n\n";
		exit 99
	}
...
#
		...
		if ( scalar(@fslist) <= 0 )
		{
			print "\n\nERROR: No file systems to process for array $bacluster. Exiting.\n\n";
			exit 98
		}
...
#
		foreach $fs (@fslist)
		{
			...
			else
			{
				...
				$fs_usage = int($fs_usage);
				$ss_usage = int($ss_usage);
...
##
	my $lr_timestamp = time();
  	...
  	$insert = "psql -U postgres -d watcher -h localhost -c \"insert into ba_lastrun (epochtime) values (to_timestamp($lr_timestamp))\"";

  	if ( $flags{'debug'} )
  	{        # do not populate the database when debugging
  		print "\nba_lastrun insert command\n";
  		print Dumper $insert;
  	}
  	else
  	{
